...
const CardSwap = ({
    ...
    // Auto-scale the stack to fit within the available parent size without clipping
    useEffect(() => {
        const updateScale = () => {
            const node = containerRef.current;
            ...
            const estimatedWidth = width + (total - 1) * cardDistance + 40; // padding allowance
            const estimatedHeight = height + (total - 1) * verticalDistance + 40;
            const fitWidth = parent.clientWidth / estimatedWidth;
            const fitHeight = (parent.clientHeight || estimatedHeight) / estimatedHeight;
            const nextScale = Math.min(1, fitWidth, fitHeight);
            setScale(nextScale);
        };
    ...
    }, [refs.length, width, height, cardDistance, verticalDistance]);
    ...
    return (
        <div
            ref={containerRef}
            className="relative mx-auto origin-center perspective-[900px] overflow-visible max-w-full"
